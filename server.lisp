...
(ql:quickload '(hunchentoot cl-prevalence cl-html5-parser drakma cl-ppcre))
(defpackage whereisjob
  (:use :cl :hunchentoot :html5-parser :cl-ppcre))
...
(defun get-job-dom(id)
  (format t "REQUESTING PAGE... ~A ~A" id #\newline)
  (multiple-value-bind (job-html)
      (drakma:http-request (concatenate 'string "http://v2ex.com/t/" id))
...
(defun get-source (root-node)
  "this function maps its way through the DOM nodes till it finds a 
   node with name 'div' and 'class' attribute equal to 'topic_content', 
   then returns that node"
  (flex-dom-map #'standard-recurse-p
                (lambda (node)
                  (if (equalp (node-name node) "link")
                      (if (equalp (element-attribute node "rel") "canonical")
                          (return-from get-source (element-attribute node "href")))))
                root-node))
...
(defun parse-job(job-dom id user)
