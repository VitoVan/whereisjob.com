...
(or (> (length (cl-prevalence:find-all-objects *p-system* 'job)) 0)
...
(defun get-jobs-dom(page)
  ...
  (multiple-value-bind (jobs-html response-code)
...
(defun get-yesterday-job-list()
  (do* ((page 0 (+ 1 page))
        ...
        (final-list nil
                    (if (not (equal job-list "error"))
                        (append job-list final-list)))
        (job-found nil (or job-found (not (null job-list)))))
       ((or (equal job-list "error") (and job-found (null job-list)))
        final-list)))

(defun get-job-dom(tid)
  (format t "REQUESTING PAGE... ~A ~A" tid #\newline)
  (multiple-value-bind (job-html response-code)
...
(defun get-email(job-dom)
  (let* ((lines (split #\newline (get-content-text job-dom)))
         (word nil))
    (dolist (line lines)
      (setf word (scan-to-strings "[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}"  line))
      (if word (return)))
    word))

(defun create-job(tid user avatar remote title cities email content)
...
(defun parse-job(job-dom tid user)
  (let* ((title (get-title job-dom))
         ...
         (email (get-email job-dom))
         (content (get-content-text job-dom)))
    (create-job tid user avatar remote title cities email content))
